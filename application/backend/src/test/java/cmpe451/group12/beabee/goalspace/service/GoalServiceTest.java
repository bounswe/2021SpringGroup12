...
public class GoalServiceTest {
    ...
    public void setUp(){
        ...
        goalService = new GoalService(goalRepository, subgoalRepository, goalPostMapper, subgoalPostMapper, subgoalShortMapper, goalGetMapper, goalShortMapper, userRepository, entitiShortMapper);
    }
    ...
    @Test
    public void whenCompleteGoalCalledWithValidRequest_ItShouldReturnSuccess(){
        Long goal_id = 1L;
        ...
        Assert.assertEquals(new MessageResponse("Goal completed successfully!", MessageType.SUCCESS), result);

        Mockito.verify(goalRepository).findById(goal_id);
        Mockito.verify(subgoalRepository).saveAll(goal1.getSubgoals().stream().collect(Collectors.toList()));
        Mockito.verify(goalRepository).save(goal1);
    }
