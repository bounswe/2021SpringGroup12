...
import requests
import sqlite3
from db.mapper import create_anime_mapper
import json
...
def add_mal_anime_to_db(anime):
    try:
        ...
        cur.execute(
            ...
            (anime["title"], anime["mal_id"], anime["episodes"], anime["image"], anime["airing"], anime["start_date"], anime["end_date"],
             anime["score"], anime["rating"], anime["type"], anime["synopsis"], anime["duration"], sequel_id, prequel_id)
        )
        anime_data = cur.execute(
            "SELECT id FROM MalAnimes WHERE title = ? AND mal_id = ?", (anime["title"], anime["mal_id"]))
        anime_id = anime_data.fetchone()[0]
        for genre in anime["genres"]:
            cur.execute(
                "INSERT OR IGNORE INTO Genres(name) VALUES (?)", (genre,))
            genre_data = cur.execute(
                "SELECT id from Genres WHERE name = ?", (genre,))
            genre_id = genre_data.fetchone()[0]
            cur.execute(
                "INSERT OR IGNORE INTO GenreRelation(anime_id, genre_id) VALUES (?,?)", (anime_id, genre_id))
        con.commit()
        con.close()
...
def validate_and_map_user_anime(body):
    fields = ["title", "episodes", "image", "airing", "start_date",
              "end_date", "score", "rating", "type", "synopsis"]
    for field in fields:
    ...
    try:
        ...
        for err in error.errors():
            ...
            if type == "none":
                errors.append(
                    {"field": err["loc"][0], "msg": "Field cannot be empty"})
            else:
                errors.append(
                    {"field": err["loc"][0], "msg": "Please enter a valid "+type})
